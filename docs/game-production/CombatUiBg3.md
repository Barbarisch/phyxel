...
| 5 | Click-to-move outside combat (G-75) | left click on ground = path + walk; NavGraph route | L3 probe + L4 | DONE 2026-09-15: `Core::ClickToMove` (7 unit tests) + pointer-driven shell (free cursor, LMB = pointer click, NPC click walks+talks, WASD/dialogue/combat cancel), `walk_to`/`walk_status`/`pointer_click` API; L4 on the probe build (all legs). Found: G-108 dialogue HUD (fixed), G-109 unprojection (fixed), G-110 screen_of (open). |
...
- 2026-09-15 — Increment 5: click-to-move L4 (see G-75); the harness can now walk legs with `walk_to` instead of injected keys (not yet switched over — a follow-up).

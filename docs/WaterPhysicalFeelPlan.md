...
| ✅ | **ENTITY COUPLING — water responds to and resists things** (small-scale Phase 4, 2026-07-31). **4.1** `WaterManager::sampleWater`/`submergedFraction` — THE shared water-query surface (fill+floor-aware sim walk in-region, table→implicit-sea fallback outside; `cameraSubmergence` delegates). **4.2** buoyancy + water drag on `VoxelDynamicsWorld` bodies via an injected AABB-submersion callback; per-body `buoyancy` density ratio; wet bodies never sleep, slept bodies re-check water ~1 Hz staggered (the hover-asleep trap). **4.3** wading on `AnimatedVoxelCharacter` via injected `WaterHooks` (depth/addRipple/splash): depth-scaled slowdown (hip-deep ≈ halves pace, capsule-scaled), stride-cadence footstep rings, entry-splash burst (waterfall-mist pattern, splash-tuned, 0.25 s/entity cooldown). **v1 limitation: hooks wired for the PLAYER factory (`createAnimatedCharacter`) only — NPCEntity characters need the same wiring (small follow-up).** Swimming deferred (user decision). | 4.1: 4 exact-depth tests + live parity (underwater fog unchanged; camera reads identical sim data — the "+1 cell" over naive seaLevel−y is the pre-existing cell-containing-seaLevel fill semantic). 4.2: 4 tests green first run incl. dry bit-parity + sleep/wake; **L4: a crate activated at the SEABED rose and floats dead-stable at submerged fraction 0.625 = exactly 1/1.6** (`waterlab_floating_crate_20260731.png`); a 0.3-deep puddle correctly does NOT float it. 4.3 **L4: walk speed 1.70 → 1.00 u/s (−41%) crossing dry → depth-1 water; stride rings + entry splash visible** (`waterlab_wading_rings_20260731.png`, `waterlab_entry_splash_20260731.png`). |
...
| ✅ | **WATER LAYER RENDERED EVERYWHERE** (water-as-terrain-stage P1, 2026-07-31, `ebcb2460`) — the hydrology bake's per-column basin levels are a first-class render input: `HydrologyMap` exports `originX/originZ/cellSize/levels()`; `WaterRenderPipeline` set-1 binding 3 = 256² R32F level texture (**NEAREST** — basins are piecewise-constant, bilinear would tilt water across divides; 1×1 sentinel keeps no-bake worlds in flat-sea mode, `invCellSize==0`); `water.vert` puts each sheet vertex at its column's basin level (lakes at spill, sea at sea level, lakes get 0.2× swell); RenderCoordinator uploads on `hydrology()` pointer change (memoized bake → fires once per world). **The old render-slab defect (below) is closed for seas + lakes.** Rivers/creeks stay near-field cell-rendered in v1 — a piecewise-flat 256² grid cannot represent a sloping channel. `WaterManager` caches the shoreline-snapped table grid (`m_tableLvlLocal`) and suppresses per-cell rendering of source-pinned cells AT their column's snapped level (the layer draws them); splashes/pours/river pins keep cell rendering. **Accepted v1 regressions:** suppressed lakes lose RippleField response (fast-follow: sample the ripple texture in `water.vert`); lake sheet edges are 128-u cell-stepped at range. **Rim-wall trap (cost hours):** clipmap quads connecting basins at different levels are vertical water walls; the depth-based dry-land gate misses them against sky, and one near the camera washes the WHOLE frame pale blue. Fix = discard `\|fragY − restLevelY\| > 2.5·amp + 0.5` (own-column band); an above-only test was NOT enough — the wall half over the high basin's columns sits *below* that basin's level and survived it. | Red→green: `BakedWaterTableFillsLakeAndSurvivesRecenter` flipped (undisturbed baked lake → **zero** surface cells; splash above it still cell-renders, then drains); 82/82 water suites. **L4:** WaterLab flat mode pixel-equivalent at the reference camera (137 vs 132 FPS); RiverLab perched lake y≈325 drawn AT RANGE at its own level lapping the mountainside, gorge/valleys dry — the exact case that killed the flat-plane attempt; CreekLab creek cells still cell-render (53 surface cells with the region over the wet segment). **Verify-time gotchas:** editor near-chunk render distance + the *vertical* streaming band mean a high camera can have ZERO visible chunks — the "all-water wash" was sky+sheet with no terrain drawable, not a water bug; `visible_chunk_count`/`mesh_timing` in `get_render_stats` disambiguates in seconds. Drive-by fix in the same commit: `scanForMirrorVoxels` consumes LAST frame's cull indices — mass chunk unload (camera teleport) left stale out-of-range indices → Debug CRT assert as an INVISIBLE modal dialog = "engine hang with API answering" (silent UB in Release); now bounds-checked. |

| ✅ | **CREEKS SHAPED BY THE TERRAIN** (water-as-terrain-stage P2, 2026-07-31, `7632516f`) — user directive delivered: orders 1-2 lie in a **bounded parabolic swale** (~1.6 voxels at the centreline over a 3-voxel half-band; bounded in ABSOLUTE voxels — a fractional relief attenuation measured 55-voxel slot canyons in mountains) and their **inner band generates a 2/3-subcube bed shelf** (`ColumnSample::creekBed`, same 0.15 threshold as the runtime pin) so the ribbon rests 1/3 voxel below its banks. `FlowField::nearestChannel` gained `minOrder` (default 3 → valley shaping unchanged). **MEANDER-ALIGNMENT bug found by the shelf (latent since the meander landed):** the runtime bound `FlowField::channelAt` on RAW coordinates while the generator carves the WARPED line (up to ~55 u apart) — raw covers only **33%** of the carve; ribbons sat beside their beds. `WorldGenerator::channelHitAt/channelFlowDirAt` (one shared warp helper) now feed the editor's river depth/order/flow bindings + the `water_table_level`/`water_find_river` probes. Creek lines now **WIND** — the "straight cell-centre line" geometry note is obsolete. | Red→green ×3 with measured red baselines (swale 0.205→1.503 bank-minus-creek voxels; creekBed never set → shelf 2/3 + bank control solid; warped-vs-raw carve coverage 100% vs 33% regression guard). Full suite 3052/3055 (2 pre-existing LLM-network fails). **L4 (CreekLab regenerated; old DB in `worlds/pre_p2_backup/`):** ribbon meanders inside its swale at the reference vantage (`creeklab_creek_meander_swale_recess_p2_20260731.png` vs the 07-30 straight strip); `water_probe` floor=0.667 under pins on fresh AND DB-round-tripped terrain; footprint confined. Trap: a probe right after launch can read floor 0.0 before the stream-in solidity resync — recenter/wait, then trust it. |

| ✅ | **POURED WATER PERSISTS** (water-as-terrain-stage P3, 2026-07-31, `72cd7344`) — hand-poured ponds no longer vanish when the window recenters (incl. VERTICALLY — how the demo pours first drained). `recenter()` captures departing columns' unpinned surface levels into a sparse world-keyed store; `rebuildOcean()` reseeds+erases in-window overrides (table double-count guard; unloaded-ground columns keep theirs); `save_world` → `world_meta["water_overrides"]` → restored on world open. Monotone by construction (reseed ≤ captured level ≤ reseeded level) — cycles cannot grow water. Pinned bake water is never captured. | Red baseline = the old `RecenterPastThePoolDropsItAtTheFrontier` (the green pin of the silent drop, now replaced); 5 new tests over a chunk-backed basin (XZ + vertical round trips, double-count guard, serialize round-trip + garbage rejection, 10-cycle never-grows stress asserted per cycle); 74/74 water suites. **L4 CreekLab:** 3.0-mass pond → walk to x=500 (mass 0) → back = 2.86 restored same columns → `save_world` + full engine restart + walk back = 2.86 again from `world_meta` (`creeklab_pond_survives_restart_p3_20260731.png`). Boot note: a saveAll'd CreekLab takes minutes to boot (`buildNavGrid` over ~1500 loaded chunks — pre-existing, not water). |

| ✅ | **CA EDGE OUTFLOW — the window edge is no longer a wall** (water-as-terrain-stage P4, 2026-07-31, `96c3a048` — **the 4-phase plan is COMPLETE**) — `WaterSimulation::setEdgeOutflow`: ring cells bleed unpinned water (excess above the hold; thin layers on stacks, so stacks drain top-down) at 0.25/step into a per-column accumulator; WaterManager banks it at the just-outside world column (cap 4.0/col — beyond it the outside world absorbed it), redeposits when the window covers a landing column with loaded ground, and persists the bank as `B x z mass` lines beside the P3 overrides. Pinned/channel cells EXEMPT (bleeding an infinite reservoir mints mass). Editor enables for baked worlds (`water.edgeOutflow` overrides); authored worlds keep walls. `water_stats` gains `overrides`/`edge_outflow`/`outflow_bank`. | Red = legacy walls asserted in-test with the flag off; 4 new tests (seam conservation over 3 landing columns, redeposit over real chunk ground, pinned-edge no-bleed mint guard, bank serialize round-trip); 78/78 water suites. **L4 RiverLab:** gorge-river frontier spill banks and PLATEAUS at the caps (133.3 steady 75 s — settles, no runaway); a 10-unit pour against the edge leaves a 0.29 hold-film with the rest banked — the invisible wall is gone. WaterLab (no bake) correctly stays legacy. |

| ✅ | **TANGIBLE WATER — the full plan SHIPPED** (2026-07-31, phases A `cfd700ce` → E `b318b35f` → C `1d05143e` → B `b7b882d4` → D `d5f0f66c` → F `973eeac0`). **A**: `WaterBodyIndex` — CC-labeled bodies over the bake (OCEAN/LAKE ≥4 cells infinite; POND smaller), `water_bodies` cmd (RiverLab: 3,547 bodies). **E**: currents push entities — `flowAtWorld` (live CA flow + kinematic order-speed fallback, ONE shared helper with the shading) → rigid-body relative drag at the buoyancy site + `WaterHooks.flowAt` wading push + **NPC hooks gap closed** (all 4 NPCManager spawn sites); L4: crate drifted 22 u downstream the gorge river. **C**: finite-body machinery — pin exclusion pre-snap + rim guard, column-total bottom-up deficit hydration (per-cell top-up minted ~0.19/rebuild; top-cell dump sloshed +5.6/cycle — both measured), per-BODY level deltas (F1: per-column records re-mint scooped ponds), no-bleed mask (F3), `sampleWater` delta fallback (F4), `D id delta` store lines (same world_meta key, v1-compatible). ⚑**Bake-cell "ponds" CANNOT be finite** — coarse levels fragment against fine terrain; measured 6.5k-mass → 17.8 round trip; bake bodies stay pinned. **B**: fine-scale ponds EXIST — `discoverFinePonds` bounded border-discard depression fill (seam-free by construction), spill−0.15 freeboard (cannot leak), 4–200 cols, depth ≥1.5; WorldGenerator registry + `finePondAt`; THE finite bodies. L4: a natural 183-column pond hydrated to EXACTLY its registry columns. **D**: `scoopWater` — pinned refills (infinite), finite drops its record by removed/areaColumns (recording the scooped column's transient surface diverged by the re-leveling — measured 15.6-mass loss); L4: 28.9 scooped mass conserved to the gram, level persisted across walk-away, `D 1099528405504 -0.189` in world_meta. **F**: wave energy ∝ body size (RG32F level+energy texture, log-area mapping, replaces binary 0.2×); planned far-edge smoothing DROPPED as vacuous (flat bodies make level filtering a no-op); ripple-on-far-layer stays the fast-follow. CreekLab note: gentle relief yields no depth≥1.5 fine ponds — pond density on soft terrain = the recorded dish-carve knob. Wading-push L4 deferred to a lab with a walkable river reach (code path shared with verified wading, unit-pinned). | Suites: 103/103 water + 140/140 water+terrain+physics gates; every phase red-before-green with measured red baselines. |

### THE open defect — CLOSED for seas + lakes (P1, above); still open for far RIVERS
...
are ruled out with evidence in §2c/§2d — read those before proposing one. **P1 (2026-07-31) closes
this for seas and lakes via the per-column level texture; far rivers remain near-field-only and need
their own representation (backlog).**

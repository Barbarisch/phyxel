...
| **Settings** | ✅ DONE — data-driven `settings:*` overlay (`loadGameScreenInto(ui,"settings",…)` from `resources/ui/settings_screen.json`); replaces ImGui `renderSettingsScreen`. Standard **Graphics** (Resolution/V-Sync/Fullscreen/FOV) / **Audio** (Master/Music/SFX) / **Controls** (Mouse Sensitivity). `buildMenuElement` now builds slider/checkbox/dropdown bound BIDIRECTIONALLY to `GameSettings` via new `MenuActions::onGetSetting`/`onSetSetting` (floats; checkbox=0/1, dropdown=index); `back` action via `onBack`. Changes apply live; host saves on Back. Verified live on `R5Verify` (widgets show current values; FOV slider 45→111 applied; Back→pause). **Deferred:** keybinding rebind (key-capture), brightness + invert-Y (no live-apply path), AI settings (dev-only). **Polish follow-ups:** open-dropdown overlap; layout is tall (bottom rows near the client edge at 125% DPI). |

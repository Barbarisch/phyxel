...
| **Screens** (Intro/Victory/Credits) | `GameMenus.cpp` `renderIntro/Victory/CreditsScreen` (ImGui) | `UISystem` screens driven by `ScreenState` (follow the pause-menu pattern: a `load<Screen>Into` builder + `<screen>:*` namespace) |

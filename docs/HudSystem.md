...
- [ ] **Menu `transition_scene` click didn't fire the scene transition** (open/close-submenu worked;
  ...
  `transition_scene` → `SceneManager::transitionTo`, and the menu renders at boot, so suspect the
  injected click landed on the panel not the button, or queued `ui_click`s racing the scene pump.

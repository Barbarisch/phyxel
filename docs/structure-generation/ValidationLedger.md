...
| 24 | place_path | M | **L3** | 🟡 **L3 proven; L4 hugs terrain, stamp paving owed** | 2/2/1 = **5** | `planStraightRamp`/`planSwitchback`/`planSettlementPaths`/**`planTerrainPath`** L3-proven (`PathPlannerTest`+`SettlementPathsTest`: TraversalProbe walks them incl. `TerrainPathHugsHillWhereStraightTunnels` — tf cut ≤9 vs linear ≥30; teeth + negatives, red-before-green). **`build_settlement` uses the TERRAIN-FOLLOWING grader** (slope-limited tube hugging the surface): live hills 19/19 edges, breakdown now **81% level (on terrain), 16% cut (small/local), 3% fill** (was 77% buried with linear). Owed: faithful **cut removal** (removeMicrocube above S at the 16% transition cells) + **level paving** (cobblestone cap, the microcube-density work) for full walkability/visibility; runtime walkability probe over the stamped chunks. |

...
| 24 | place_path | M | **L3** | 🟡 **L3 + L4-visible** | 2/2/1 = **5** | `planStraightRamp` + `planSwitchback` + `planSettlementPaths` (MST over building doors, each edge graded with flat aprons); **`PathPlannerTest` + `SettlementPathsTest` L3** (TraversalProbe walks ramp/switchback/whole network; cliff/terminus/terraced-cube teeth, flat-stamp negative control, too-short/too-tight/degenerate reported, red-before-green). **L4 wired (3c-2):** `build_settlement` computes the network from live terrain (runtime 19/19 edges) and STAMPS the graded surfaces as Cobblestone paving (26614 microcubes, visible in-world — screenshot). **Owed:** runtime per-voxel walkability probe over the STAMPED chunks (live L3 over micro-occupancy) + faithful cut-through-terrain (current cut is best-effort; fill/level faithful) |

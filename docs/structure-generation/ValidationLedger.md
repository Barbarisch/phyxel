...
| 24 | place_path | M | **L3** | 🟠 **L3 (plan) only; L4 stamp PARTIAL** | 2/2/1 = **5** | `planStraightRamp`/`planSwitchback`/`planSettlementPaths` are L3-proven (`PathPlannerTest`+`SettlementPathsTest`: TraversalProbe walks them on SYNTHETIC terrain; teeth + negatives, red-before-green). **`build_settlement` computes the network from live terrain (19/19 edges) but the voxel stamp is INCOMPLETE on rolling hills:** the planner grades LINEARLY between door heights (ignores intermediate terrain), so on real hills 77% of cells are CUT (path below terrain) — only FILL is stamped (3494 µcubes), level uses terrain (2447), **cut (20231) is buried/unpaved/NOT walkable**. **Real gap surfaced: needs a TERRAIN-FOLLOWING grader (hug the surface, regrade only the cube-steps) + faithful cut removal** — linear anchor-ramps suit open-space (cliff/switchback) not surface paths. Owed: terrain-following path grader; cut removal; runtime walkability probe over stamped chunks. |

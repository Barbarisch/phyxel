...
| 12 | place_stairs | D | **L3** | **L3** ✅ (`AgentCanClimbSwitchbackToTopFloor` + gate + clearance; **M2 2026-08-07 `StairFallShaftTest`**: sp.ok HONORED — a failed plan no longer cuts a fall shaft or silently seals the upper story (it was doing BOTH: the realizer never read `StairPlan::ok` and pushed a StairRecord for unbuilt stairs); one bounded repair with the alternate form in the same well, else the SHELL REFUSES; stairwell-perimeter GUARD CURBS (8-micro ≈ 0.89 m, IRC R312.1.2) emitted from the drop invariant, entry-gated at stacked-well flight mouths so circulation survives (3-story straight reach re-proven); `maxStepMicro` = shared `kCharacterStepUpMicro`, no more realizer literal; StairRecord.form = the form actually BUILT) | 2/2/2 = **6** | ✅ met, no longer overstated: the old “EXEMPLAR” verdict covered only “agent climbs a well-formed switchback” and hid the silent-failure paths. OWED: mid-flight side HANDRAILS (curbs guard the shaft only), validator riser still assumes floor thickness 3 (realizer uses floorTopByStory), L-shaped/newel forms, cellar down-stair (#35), layout-aware well siting (M6) |
...
| 14 | place_chimney | **D** | L2 | **L2** ✅ (`ChimneyIntegrityTest` — measured on the REALIZED CANVAS, not on a planner's return value. **F1** the flue is CONTINUOUS AIR from firebox to cap, proven by a FLOOD FILL through air (the retired `ChimneyPlannerTest` sampled a column, so it could not see that the stack rested on a SOLID template mantel — there was no fire-to-flue path at all); **F2** clears the ridge measured on the same shell WITHOUT a chimney, by the grounded ≥2 ft (`HearthForge::kRidgeClearanceMicro` = 6 micro, IRC R1003.9, pinned ≥ the 0.610 m floor); **F3** solid masonry ring at every course + open flue inside it; **F4** an upper floor slab YIELDS to the flue (control: intact slab one cell outside the stack); **F5** a stack that would rise through the middle of an upstairs room REFUSES the shell (control: against a wall it builds). RED PROVEN BY MUTATION (the old stamped path no longer exists to fail): throat removed → F1 fails; flue carve skipped → F1/F3/F4 fail; clearance 6→3 → F2 fails; F5 stays green throughout. Plus `HearthForgeTest`: body dims vs the `object_dimensions.json` canon, stack fits its body, siting pose == the furnish pass's pose field-for-field, determinism + idempotence, stack column reserved upstairs) | 0/0/1 = **1** | ✅ The placer MOVED INTO THE SHELL 2026-08-10 (`HearthForge`, [`../ChimneyForgePlan.md`](ChimneyForgePlan.md)): hearths are sited by the floorplan as program fixtures, and body + stack are painted into the `MicroCanvas` at realize time — so the pass DISPLACES NOTHING (was ~600 cells/tavern) and the canvas detectors can finally see the chimney. The old OWED item ("an L2 scan of the PLACED chunk voxels") is RETIRED: there is no separately stamped artifact any more, the stack IS the shell. STILL OWED: firebox-area + stack-wall-thickness grounding (`GroundingGaps.md` #6/#7); roof-penetration flashing; SMOKE VFX (the plume); the hearth TYPOLOGY family (inglenook / stone / brick by style) — archetype sheet first |
...
| 18 | place_lights | **D** | L1 | **L2** ✅ (`LightingPassTest` + L4) | 0/1/1 = **2** | ✅ **CLOSED 2026-08-08 (StructureForge M5)** — was the furnishing tier's only L0 row. Lamps were `glow`-material props that self-lit their own voxels and illuminated NOTHING (zero engine point lights from structure gen; a "lit" tavern was black at night). Now the lighting pass registers a REAL point light per emitting fixture — candle_stand / wall_lantern / chandelier **and the hearth fire** — at its flame height, through a `Deps::addPointLight` callback (core must not link graphics/). `lights_registered` + `dark_rooms` reported. L4: generated tavern registers a chandelier (1.00,0.72,0.42 @ 2.6, r 9.0) and a hearth fire (1.00,0.62,0.32 @ 2.0, r 7.5) — verified via `list_lights`. GROUNDED: flame colour (~1700–1850 K) + relative brightness by candle count. OWED: light PERSISTENCE (LightManager lights do not survive save/load — engine gap), absolute intensity/radius are disclosed engine-unit tuning (no candela/inverse-square in the renderer), daylight is "has a window portal" not a real calculation, and no time-of-day gating. See StructurePipelineGaps.md |
| 19 | place_clutter | M | L1 | **L2** ✅ (2026-08-07, commit `908a911c`: surface clutter converted to **item props** — `ItemPropManager::spawnProp` static-first at MEASURED surface height (`FurniturePlacer::templateTopUnits` over the supporting table's occupied tiers, not `floorY+1`), per-purpose `surface_items` recipe sets, `setParent` cascade. `ItemPlacementTest`: recipe→holdable red-first, measured-height red vs the floor+1 guess, removal-cascade control. L4: tavern tankards ON the taproom table at y=18.12, rebuild-at-origin idempotent — 89 placed objects flat, old items swept with tavern_2) | 0/0/1 = **1** | **`d279683a` closes both owed items:** `as:"item"` recipe realization shipped (rugs → rug_woven props, L4 upstairs chambers) and `placeSurfaceItems` scatters on a min-spacing grid measured from the ACTUAL placed tabletop (rotation-correct, rim-inset — `SurfaceSpotsKeepMinimumSpacing` + on-top/rotation tests). Remaining: candle_stand → item conversion (scale mismatch — floor stand vs table candlestick); barrels/signs stay baked (no fine item equivalents) |
...
- **Settlement (38–49):** layout placers → **L2** (no overlap, fit): **`site_settlement` 38 — ✅ L2 at
  ...
  a stub orchestrator). **`subdivide_plots` 40 — ✅ L2** (`SettlementLayoutTest`: plots no-overlap + street-gap + fit + min;

...
| 19 | place_clutter | M | L1 | **L2** ✅ (2026-08-07, commit `908a911c`: surface clutter converted to **item props** — `ItemPropManager::spawnProp` static-first at MEASURED surface height (`FurniturePlacer::templateTopUnits` over the supporting table's occupied tiers, not `floorY+1`), per-purpose `surface_items` recipe sets, `setParent` cascade. `ItemPlacementTest`: recipe→holdable red-first, measured-height red vs the floor+1 guess, removal-cascade control. L4: tavern tankards ON the taproom table at y=18.12, rebuild-at-origin idempotent — 89 placed objects flat, old items swept with tavern_2) | 0/0/1 = **1** | remaining: `as:"item"` recipe schema for standalone FLOOR items (rugs, candlestick); pairwise no-overlap scan (spots are deterministic-seeded, unproven disjoint) |

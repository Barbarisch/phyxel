...
| 22 | place_fence | M | L2 | **L2+L3** ✅ (`planParcelFence` + `ParcelFenceTest`: encloses the whole perimeter minus a gate; **L3 TraversalProbe enters only via the gate, sealing it → yard unreachable (teeth)**; too-small→ok=false, red-before-green) | 1/1/1 = **3** | ✅ fences enclose with a gate; wired into `build_settlement` (gate faces settlement centre, Log cubes seated on terrain — runtime 14 parcels/588 posts, screenshot). Owed: gate precisely aligned to the path edge; posts+rails look (cubes today) |

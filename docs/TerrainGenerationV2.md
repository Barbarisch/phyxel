...
> **Snow** material (Ice reads glassy for caps). *(✅ done — Increment 4: Snow + SnowGrass.)*
>
...
> to snow by the lapse rule (physically correct — it's snow-covered); its distinctness is being
> treeless. (3) No GrassJungle / Mud materials yet (Jungle reuses GrassForest). *(Snow/SnowGrass ✅
> Increment 4 — the lapse override + Snow biome now surface SnowGrass/Snow, not Ice.)*
...
> superseded — Increment 4 adds a distinct colder `kTreelineTemp01`: SnowGrass taiga below it, bare Snow above.)*
...
>
> **Increment 4 ✅ IMPLEMENTED (2026-07-13): Snow + SnowGrass materials retire the "Ice" snow
> placeholder + a distinct treeline.** The lapse-rate override emitted the glassy **Ice** material for
> snow (Increments 1–2 stopgap). Now two grounded surfaces: **SnowGrass** (snow-dusted taiga ground —
> white top, snow-crust-over-dirt sides; the Snow biome's base surface; KEEPS conifers) and **Snow**
> (bare permanent snowpack; the alpine cap above the treeline; BLOCKS flora). Adds a second, colder
> threshold `kTreelineTemp01` (−8 °C ≈ −0.086 norm; Körner alpine-treeline synthesis) BELOW the 0 °C
> snow line `kSnowTemp01`, so the override now bands forest → **SnowGrass** (snow line, taiga) →
> **Snow** (treeline, bare cap) — retiring "treeline == snowline" (old limitation #5). Materials
> grounded (grounding-auditor): settled-snowpack density (mass 0.30 vs Ice 0.9), packed-snow friction
> 0.20, inelastic restitution 0.10, near-neutral albedo + matte roughness (Warren 1982; matte enforced
> by shipping no `_nr` sidecar so the engine default 0.9 applies). Textures generated by
> `tools/gen_snow_textures.py`. Flora gate now blocks bare **Snow** only (SnowGrass taiga keeps trees).
> Measured (TerrainMaterialTest + TerrainBiomeTest, real output): snowAlt 40,129 / warmLowSnow 0 /
> highGrass 0; bareSnow 14,695 with 0 treeline violations (every bare-Snow column below −8 °C
> effective); flora placements 128 with onSnow 0 (no trees on the bare cap) + onTaiga 83 (conifers DO
> grow on SnowGrass). Full suite 2775 tests / 0 failures (MaterialRegistry counts synced 99→101). L4:
> the engine Mountains generator placed SnowGrass on a cold peak (scan visible:true, zero Ice),
> rendering as matte white; a Snow-vs-SnowGrass patch confirms pure-white-all-faces vs white-top/
> dirt-crust-sides. Red→green: the old Ice assertions failed on the new code (iceColumns=0);
> solution-auditor PASS (read-only, working tree left untouched).

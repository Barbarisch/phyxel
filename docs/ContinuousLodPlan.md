...
| **Water** | Two tiers already half-designed: CA sim region near, **baked water table as a static surface** far (`WaterSystemV2` Phase B, unbuilt). Render tier = coarse surface quads on the same ladder | metric only | **Medium** | ⚠️ **Phase B/LOD is the user's other session — hands off** ([[water-system-v2]]). This plan supplies the metric and stops there |

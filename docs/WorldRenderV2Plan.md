...
| **M3** ✅ **SHIPPED 2026-08-11** | Aerial perspective + unified sky ambient + tone map — delivered, and superseded in scope: rather than a hand-tuned hemisphere tint and a constant haze colour, a physical scattering model (`graphics/Atmosphere.h`, `shaders/atmosphere.glsl`) now supplies the sun colour, sky ambient, haze endpoints AND a rendered sun and moon. Tone map is **AgX**, not the ACES this row originally specified. See [LightingPipeline.md](LightingPipeline.md). **AA (FXAA/TAA) is NOT part of what shipped** and remains open. | Measured: noon exterior 0.00% clipped, hearth interior 0.00% clipped (was 29.72%), exposure sweep 0.5–16 clipping 0.00% at every setting |

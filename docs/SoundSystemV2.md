...
| **3D audio** | `AudioSystem.cpp:91` (`update`), `:146` (`playSound3D`) | Spatialization wired end-to-end per source: per-sound position/velocity, listener pos/dir/up/velocity pushed every frame from the camera (`Application.cpp:3875`, line drifted from `:3728` as of 2026-07-21). | Per source; not runtime-audited |
...
| **API/MCP** | `EngineAPIServer.cpp` (audio routes now at `:4050-4059` music + `:4356-4383` sounds/play/volume — the file has drifted; the old `:3937-4259` citation no longer covers them), handlers `Application.cpp:1461` (list_sounds), `:5978-6013` (get_music_state/control_music/set_volume), `:12758-12781` (play_sound/set_volume — line numbers drifted from `:11547/:11565` as of 2026-07-21) | `list_sounds`, `play_sound` (2D/3D), `set_volume`, `get_music_state`, `control_music`. Python bindings for play/preload (`Bindings.cpp:142-151`). | Works |
| **Assets** | `resources/sounds/` | **5 WAVs** (verified 2026-07-21, up from 4): `click.wav`, `place.wav`, `test.wav`, `whoosh.wav`, `axe_chop.wav` (new — generated by `tools/gen_chop_sound.py` and now wired to gameplay, see below). No catalog, no provenance file. | The actual gap |

Gameplay currently emits sound from **at least three** call sites (was two as of 2026-07-09):
voxel place → `place.wav` (`VoxelInteractionSystem.cpp:314,323,332`); furniture activation →
`hit.wav` (`VoxelInteractionSystem.cpp:572`) — **which still does not exist on disk** (silent dead
reference, unchanged); and **new**: axe/chop impact → `axe_chop.wav` (trunk hits) / `whoosh.wav`
(leaf swipes) at `Application.cpp:8376` (`playSound3D`, part of the destruction/chop-kerf work on
`destruction-system-v2`) — this sound **does exist on disk** and is a real break-adjacent SFX, so
"no break... sounds anywhere" below is now only true for generic material-break audio, not
tree-chop impacts specifically. TTS plays on the Voice channel (`TTSService.cpp:294`). Still no
footstep, combat, door, ambience, or weather sounds anywhere; no sound catalog, `SoundRegistry`,
`AmbienceDirector`, `MusicDirector`, or biome-ambience data exist yet (all re-verified absent
2026-07-21) — the P0-P3 plan below remains not-yet-started.
...
6. **Biome-at-player exists in C++ but not over the API.**
   `WorldGenerator::sampleSurface(x,z)` (`WorldGenerator.h:155`, drifted from `:148`) returns
   ...
   (`Application.cpp:13716`, drifted from `:12462` — re-verified 2026-07-21, still just walks Y
   downward via `hasVoxelAt`, no biome data) ignores it, so nothing outside C++ can ask "what biome
   is here." No `get_biome_at` MCP tool exists yet either (re-verified 2026-07-21).
...
8. **`play_sound` hard-prefixes `resources/sounds/`** (`Application.cpp:12762`, line drifted from
   `:11551` as of 2026-07-21 — the hardcode itself is unchanged and still unfixed) — a library

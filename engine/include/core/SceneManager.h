...
struct SceneCallbacks {
    ...
    /// Called as soon as a transition is accepted, before the old scene unloads
    /// (the runtime drops injected input here so nothing is "held" across scenes).
    std::function<void()> onTransitionBegin;

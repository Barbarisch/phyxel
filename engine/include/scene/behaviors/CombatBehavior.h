...
private:
    ...
    float m_attackRange    = 1.8f;   ///< stop & attack within this range — MUST be < m_attackReach so swings connect
    ...
    float m_recoverTime    = 0.5f;   ///< brief back-off after a swing
    float m_attackDamage   = 8.0f;
    float m_attackReach    = 2.6f;   ///< hit-detection reach at the swing's hit frame
    ...
    float m_evadeChance    = 0.7f;   ///< chance to react to a given incoming swing (so fights resolve)

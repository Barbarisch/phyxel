...
void EngineAPIServer::setupRoutes() {
    ...
    // ====================================================================
    srv.Post("/api/debug/render_distance", [this](const httplib::Request& req, httplib::Response& res) {
        try {
            json params = json::parse(req.body);
            json result = queueAndWait("set_render_distance", params);
            res.set_content(result.dump(), "application/json");
        } catch (const json::exception& e) {
            json err = {{"error", "Invalid JSON"}, {"detail", e.what()}};
            res.status = 400;
    ...
    srv.Post("/api/ui/hover", [this](const httplib::Request& req, httplib::Response& res) {
        try {
            json params = json::parse(req.body);
            json result = queueAndWait("ui_hover", params);
            res.set_content(result.dump(), "application/json");
        } catch (const json::exception& e) {
            json err = {{"error", "Invalid JSON"}, {"detail", e.what()}};
            res.status = 400;
    ...
    // POST /api/ui/hover moves the pointer WITHOUT clicking and returns the tooltip shown
    // there (G-148) - an automated run has no mouse, so hover text is otherwise
    // unverifiable except by a human reading a screenshot.
    ...
    srv.Post("/api/debug/water_gpu",      waterEndpoint("water_gpu"));
    ...
    srv.Post("/api/spell/cast", [this](const httplib::Request& req, httplib::Response& res) {
        try {
            json params = json::parse(req.body);
            json result = queueAndWait("cast_spell", params);
            res.set_content(result.dump(), "application/json");
        } catch (const json::exception& e) {
            res.status = 400;
            res.set_content(json{{"error", "Invalid JSON"}, {"detail", e.what()}}.dump(), "application/json");

...
// ════════════════════════════════════════════════════════════════
// Embedded 8x16 bitmap font data (printable ASCII 32..126).
// Each character is 16 bytes (one byte per row, MSB = leftmost pixel).
// Characters 0-31 and 127 are blank.
// ════════════════════════════════════════════════════════════════
// clang-format off
static const uint8_t kFontData[128][16] = {
...
bool BitmapFont::initialize(UIRenderer* renderer) {
    // Build a 128x128 R8 atlas
    std::vector<uint8_t> atlas(ATLAS_W * ATLAS_H, 0);

    // top-left pixel = solid white (for rectangle drawing)
    atlas[0] = 0xFF;
    ...
    for (int ch = 0; ch < 128; ++ch) {
        ...
        for (int y = 0; y < GLYPH_H; ++y) {
            uint8_t bits = kFontData[ch][y];
            for (int x = 0; x < GLYPH_W; ++x) {
                if (bits & (0x80 >> x)) {
                    int px = baseX + x;
                    int py = baseY + y;
                    // Don't overwrite the reserved white pixel at (0,0)
    ...
    if (!renderer->uploadFontAtlas(atlas.data(), ATLAS_W, ATLAS_H)) {
        LOG_ERROR("BitmapFont", "Failed to upload font atlas");
        return false;
    }

    initialized_ = true;
    LOG_INFO("BitmapFont", "Initialized with {}x{} atlas", ATLAS_W, ATLAS_H);
    return true;
}
...
void BitmapFont::getGlyphUV(char c, glm::vec2& uvMin, glm::vec2& uvMax) const {
    ...
    float invW = 1.0f / ATLAS_W;
    float invH = 1.0f / ATLAS_H;
    uvMin = {col * GLYPH_W * invW, row * GLYPH_H * invH};
    uvMax = {(col + 1) * GLYPH_W * invW, (row + 1) * GLYPH_H * invH};
}
...
void BitmapFont::drawText(UIRenderer* renderer, const std::string& text,
    ...
    for (char c : text) {
        ...
        getGlyphUV(c, uvMin, uvMax);
        renderer->drawQuad({x, y}, {glyphW, glyphH}, uvMin, uvMax, color);
        x += glyphW;
    }
}

...
// Shared cube mesh (position + normal + uv). Normal/uv are unused by the
// VFX shaders but kept so the vertex layout matches the debris pipeline.
struct Vertex {
    glm::vec3 pos;
    glm::vec3 normal;
    glm::vec2 texCoord;
...
VfxRenderPipeline::VfxRenderPipeline() {}

VfxRenderPipeline::~VfxRenderPipeline() {
    cleanup();
}

void VfxRenderPipeline::cleanup() {
...
void VfxRenderPipeline::initialize(VkDevice device, VkPhysicalDevice physicalDevice, VkRenderPass renderPass, VkExtent2D swapChainExtent) {
    m_device = device;
...
void VfxRenderPipeline::createBuffers() {
    // 1. Static cube vertex buffer.
    ...
    m_instanceBufferSize = sizeof(VfxInstanceData) * MAX_INSTANCES;
...
void VfxRenderPipeline::createDescriptorSetLayout() {
...
void VfxRenderPipeline::createPipeline(VkRenderPass renderPass, VkExtent2D swapChainExtent) {
    auto vertShaderCode = readFile(Core::AssetManager::instance().resolveShader("vfx.vert.spv"));
    auto fragShaderCode = readFile(Core::AssetManager::instance().resolveShader("vfx.frag.spv"));
    ...
    bindingDescriptions[1].stride = sizeof(VfxInstanceData);
    ...
    attributeDescriptions.push_back({2, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(VfxInstanceData, position)});
    attributeDescriptions.push_back({3, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(VfxInstanceData, scale)});
    attributeDescriptions.push_back({4, 1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(VfxInstanceData, color)});
    ...
    // Depth-test against the scene so particles behind geometry are occluded,
    // but do NOT write depth — additive glow must accumulate freely.
    ...
    // Additive blending (premultiplied color from the fragment shader).
...
void VfxRenderPipeline::updateInstanceBuffer(const std::vector<VfxParticle>& particles, size_t activeCount) {
    if (activeCount == 0) return;

    void* data;
    vkMapMemory(m_device, m_instanceBufferMemory, 0, m_instanceBufferSize, 0, &data);

    VfxInstanceData* instances = static_cast<VfxInstanceData*>(data);
...
void VfxRenderPipeline::render(VkCommandBuffer commandBuffer, const Camera& camera, const glm::mat4& projectionMatrix, const std::vector<VfxParticle>& particles, size_t activeCount) {
    if (activeCount == 0) return;
    ...
    // activeCount may exceed MAX_INSTANCES; clamp the draw to what we copied.
    uint32_t instanceCount = static_cast<uint32_t>(std::min<size_t>(activeCount, MAX_INSTANCES));
    vkCmdDraw(commandBuffer, static_cast<uint32_t>(CUBE_VERTICES.size()), instanceCount, 0, 0);
}

void VfxRenderPipeline::recreatePipeline(VkRenderPass renderPass, VkExtent2D swapChainExtent) {
    vkDestroyPipeline(m_device, m_pipeline, nullptr);
    createPipeline(renderPass, swapChainExtent);
}

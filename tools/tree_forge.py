...
# The multi-resolution voxel substrate (MicroVoxels canvas, hierarchical C/S/M emit, capsule/
# sphere rasterizers) is SHARED with other generators — it lives in forge_core.py.
from forge_core import (MICRO_PER_CUBE, SUB_PER_CUBE, MicroVoxels, emit,          # noqa: F401
                        rasterize_capsule, rasterize_sphere,
                        _add, _sub, _mul, _dot, _len, _norm)
...
def rasterize_branches(nodes, mv, params):
    """Draw each branch segment as a tapered capsule on the shared canvas, one resolution level
    finer than the segment's bulk — emit() re-compresses solid interiors to cubes while curved
    surfaces keep their subcube/micro shell (slopes, not stairsteps)."""
    log = params["log_mat"]
    MP = MICRO_PER_CUBE
    for nd in nodes:
        p = nd["parent"]
        ...
        rasterize_capsule(mv, a, b, ra, rb, vs, seg_mat)
...
def add_foliage(nodes, mv, rng, params):
    ...
    for nd in nodes:
        ...
        if nd["children"] and rng.random() > params["leaf_density"]:
            continue
        center = _mul(nd["pos"], MP)
        r = lr * rng.uniform(0.7, 1.15)
        rasterize_sphere(mv, center, r, vs, leaf, overwrite=False)   # wood wins: twigs poke through

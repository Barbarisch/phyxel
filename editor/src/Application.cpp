...
// build_settlement — thin glue over Core::SettlementBuildService. The composition itself
// (grounding gate, program/tier resolution, layout, site prep, buildings, residents) now
// lives in the ENGINE (engine/src/core/SettlementBuildService.cpp) so it is headless-
// testable and reachable from a shipped game; the editor's only remaining job is to drive
// the returned work units on MainThreadJobs (one per frame, [no-frozen-engine]) and to
// supply the undo hook. Pass {"async": false} to run every unit inline.
void Application::registerSettlementCommands() {
    auto& reg = m_commandRegistry;

    reg.on("build_settlement", [this](const Core::APICommand& cmd, nlohmann::json& r) {
        Core::SettlementBuildService::Deps deps;
        ...
        // async by DEFAULT: enqueue the units as a tracked MainThreadJob (progress via
        // /api/jobs + the Active Jobs overlay) and return the job id + plan immediately.
        const bool asyncBuild = cmd.params.value("async", true) && mainThreadJobs != nullptr;
        if (asyncBuild) {
            const auto jobId = mainThreadJobs->start("build_settlement", plan.jobLabel);
            for (auto& u : plan.units) mainThreadJobs->addUnit(jobId, u.label, std::move(u.run));
            // final unit: fold the phase results into the job's payload
            mainThreadJobs->addUnit(jobId, "finalizing",
                [this, jobId, paths = plan.paths, props = plan.yardProps,
                 residents = plan.residents]() {
                    if (mainThreadJobs)
                        mainThreadJobs->mergeResult(jobId, {{"paths", *paths},
                                                            {"yard_props", *props},
                                                            {"residents", *residents}});
                });
            mainThreadJobs->seal(jobId);
            r = {{"success", true}, {"job_id", jobId}, {"async", true},
                 {"settlement", plan.settlement}, {"program", plan.program},
            ...
            // ============================================================
                ...
                if (!chunkManager) {
                    response = {{"error", "ChunkManager not available"}};
                           ...
                           cmd.params.value("schema", std::string()) != "v2") {
                    response = {{"error", "Missing 'type' parameter"}};
                } else {
                    // ONE build pipeline (Structure Generation v2) for every generated
                    // building — the v1 composites (house/tavern/tower, BuildingSpec) are
                    // REMOVED. Plain house/tavern requests alias onto v2 typologies; the
                    // orchestration lives in Core::StructureBuildService so the API,
                    // GameDefinitionLoader, and settlements share one engine path.
                    // Primitives (wall/staircase/table/...) place via the shared tail.
                    ...
                    if (cmd.params.value("schema", std::string()) == "v2") {
                        response = Core::StructureBuildService::buildV2(cmd.params, deps);
                    } else if (stype == "house" || stype == "tavern") {
                        nlohmann::json v2p =
                            Core::StructureBuildService::aliasLegacyParams(cmd.params);
                        LOG_INFO_FMT("StructureBuild", "legacy '" << stype
                                     << "' aliased to v2 typology '"
                                     << v2p.value("typology", std::string()) << "'");
                        response = Core::StructureBuildService::buildV2(v2p, deps);
                        if (response.is_object()) response["aliased_to_v2"] = v2p["typology"];
                    } else if (stype == "tower" || cmd.params.contains("stories")) {
                        response = {{"error", "v1 composite generators were removed: 'tower' has "
                                              "no v2 typology yet, and BuildingSpec ('stories' "
                                              "without schema:'v2') is gone. Use schema:'v2' with "
                                              "a BuildingProgram (footprint/typology/style)."}};
                    } else {
                        auto structure = Core::StructureGenerator::generateFromJson(cmd.params);
                        response = Core::StructureBuildService::placeAndRegister(
                            structure, cmd.params, deps);
                    }
                }
